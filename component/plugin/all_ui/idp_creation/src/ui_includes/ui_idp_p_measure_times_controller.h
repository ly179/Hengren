#ifndef UI_IDP_P_MEASURE_TIMES_CONTROLLER
#define UI_IDP_P_MEASURE_TIMES_CONTROLLER
#include <QtGlobal>
...
struct UiIdpPMeasureTimesControllerPath;
}

struct cdsem::UiIdpPMeasureTimesControllerPath {
    QString proHuiDir;
    QString huiFileName;
    QString path;
#if defined(QT_DEBUG) && defined(_USE_CODE_EDITOR)
...
#endif
    UiIdpPMeasureTimesControllerPath() {
#ifdef __PRO_HUI_DIR
        proHuiDir = __PRO_HUI_DIR;
#else
        proHuiDir = ":/h_ui";
#endif
        huiFileName = "idp_p_measure_times_editor";
#if defined(QT_DEBUG) && defined(_USE_CODE_EDITOR)
        classFileName = "idp_p_measure_times_controller";
#endif
...
class cdsem::UiIdpPMeasureTimesController {
private:
    const cdsem::UiIdpPMeasureTimesControllerPath pathInfo;
public:
#if defined(QT_DEBUG) && defined(_USE_CODE_EDITOR)
    QList<CDE90LayoutScriptLineObject> __allWidgets;
#endif
    CDE90TableView* pNoTable = nullptr;
    CDE90TextFieldBox* timesTextField = nullptr;
    CDE90ButtonBox* applyBtn = nullptr;
    CDE90ButtonBox* okBtn = nullptr;
    CDE90ButtonBox* cancelBtn = nullptr;
    UiIdpPMeasureTimesController() = default;
    ~UiIdpPMeasureTimesController() noexcept {}
...
, &__allWidgets
        ...
        timesTextField = helper.get<CDE90TextFieldBox>("timesTextField");
...
#if defined(QT_DEBUG) && defined(_USE_CODE_EDITOR)
    [[nodiscard]] static CodeEditorController* createCodeEditor(const cdsem::UiIdpPMeasureTimesControllerPath& path) {
        return getCodeEditor<cdsem::UiIdpPMeasureTimesControllerPath>(path);
    }
