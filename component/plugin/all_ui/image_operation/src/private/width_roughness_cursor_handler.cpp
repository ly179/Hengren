...
WidthRoughnessCursorHandler::WidthRoughnessCursorHandler(ImageOperationControllerDataHelper* dataHelper) : dataHelper(dataHelper) {}
WidthRoughnessCursorHandler::~WidthRoughnessCursorHandler() noexcept {}

void WidthRoughnessCursorHandler::doAutoMeasure(const sem::ONE_PARAM_ARG_FUNC<const OperationResult&>& callback, const cdsem::TmpImageData& imageData, const ImageAddtionDataModel& addition) {
    ...
    MainThreadInvoker::getInvoker().runLater([image, pixelSize, this, callback]() mutable{
        ...
        AmpWidthRoughnessParam& widthRoughnessParam = cdsem::CDSEM_GlobalDataHelper::get()->getTmpAmpWidthRoughnessParam();
        collectCenters();
        if (widthRoughnessParam.acdParam.centers.size() > 0) {
            ImageResultData data = HRMat::measureRoughnessImage(image, pixelSize, widthRoughnessParam, {});
            qDebug() << "测量完毕";
...
void WidthRoughnessCursorHandler::collectCenters() {
    AmpWidthRoughnessParam& widthRoughnessParam = cdsem::CDSEM_GlobalDataHelper::get()->getTmpAmpWidthRoughnessParam();
    QList<QPoint> centers;
    QPoint center;
    if (widthRoughnessParam.cursorType == 0) {
    ...
    } else if (widthRoughnessParam.cursorType == 1){
        ...
        widthRoughnessParam.acdParam.centers.clear();
        return;
    }
    widthRoughnessParam.acdParam.centers = centers;
}

void WidthRoughnessCursorHandler::markWidthCursors() {
    AmpWidthRoughnessParam& widthRoughnessParam = cdsem::CDSEM_GlobalDataHelper::get()->getTmpAmpWidthRoughnessParam();
    cdsem::AMS_Status amsStatus = cdsem::CDSEM_GlobalDataHelper::get()->getAmsStatus();
    if (amsStatus == cdsem::AMS_SOFF && widthRoughnessParam.measurePosition == 1) {
        markWidthLineTools();
        cdsem::CDSEM_GlobalDataHelper::get()->setAmsStatus(cdsem::AMS_SASSIST);
        return;
    }
    dataHelper->recordAssistCenterPos();
    int num = widthRoughnessParam.acdParam.acdType == cdsem::ACDT_MANUAL ? widthRoughnessParam.acdParam.target : 1;
    uiHelper->canvasToolWidget->setVisible(widthRoughnessParam.acdParam.acdType == cdsem::ACDT_MANUAL && num > 1);
    if (widthRoughnessParam.cursorType == cdsem::ABCT_TWO_BOX) {
        bool needExchange = dataHelper->ampCursors.doubleBoxTools.isEmpty();
        markWidthDoubleBoxTools(widthRoughnessParam);
        if (needExchange) {
            dataHelper->exchangeCurrentAmpCursor(&dataHelper->ampCursors.doubleBoxTools);
        }
    } else if (widthRoughnessParam.cursorType == cdsem::ABCT_ONE_BOX) {
        bool needExchange = dataHelper->ampCursors.oneBoxTools.isEmpty();
        markWidthOneBoxTools(widthRoughnessParam);
        if (needExchange) {
...
void WidthRoughnessCursorHandler::markWidthDoubleBoxTools(AmpWidthRoughnessParam& widthRoughnessParam) {
    int num = widthRoughnessParam.acdParam.acdType == cdsem::ACDT_MANUAL ? widthRoughnessParam.acdParam.target : 1;
        ...
        for (int i = 0 ; i < num; ++ i) {
            ...
            if (dataHelper->ampCursors.doubleBoxTools.isEmpty()) {
                ...
                QObject::connect(measurementTool, &ImageMeasurementDoubleRectTool::sizeChanged, dataHelper->d, [this, measurementTool](){
                    ...
                    AmpWidthRoughnessParam& param = cdsem::CDSEM_GlobalDataHelper::get()->getTmpAmpWidthRoughnessParam();
            ...
            //modify direction
            // if (widthRoughnessParam.direction == 0) {
    ...
    for (int i = 0; i < num; ++ i) {
        ...
        if (widthRoughnessParam.designValue != nullptr)
            measurementTool->setDesignValue(cdsem::PixelDistanceConvert::instance()->getPixelSizeFromDistanceNM(widthRoughnessParam.designValue, IOPT->getPixelSize()));
        measurementTool->setSumLine(widthRoughnessParam.inspectArea);
        measurementTool->setSearchArea(widthRoughnessParam.searchArea);
        ...
        } else if (widthRoughnessParam.acdParam.centers.size() > 0 && i < widthRoughnessParam.acdParam.centers.size()){
            moveMeasuremntToolByCenter(measurementTool, widthRoughnessParam.acdParam.centers[i], false);
        } else {
...
void WidthRoughnessCursorHandler::markWidthOneBoxTools(AmpWidthRoughnessParam& widthRoughnessParam) {
    int num = widthRoughnessParam.acdParam.acdType == cdsem::ACDT_MANUAL ? widthRoughnessParam.acdParam.target : 1;
    ...
    for (int i = 0; i < num; ++ i) {
        ...
        if (cdsem::checkAmpWidthRoughnessObjectIsDense(widthRoughnessParam.object)) {
            width = widthRoughnessParam.denseParam.cursorSizeX;
            height = widthRoughnessParam.denseParam.cursorSizeY;
        } else {
            //modify direction
            // if (widthRoughnessParam.direction == 0) {
            height = widthRoughnessParam.inspectArea;
            width = widthRoughnessParam.searchArea + cdsem::PixelDistanceConvert::instance()->getPixelSizeFromDistanceNM(widthRoughnessParam.designValue, IOPT->getPixelSize());
            // } else {
            //     width = widthRoughnessParam.inspectArea;
            //     height = widthRoughnessParam.searchArea + cdsem::PixelDistanceConvert::instance()->getPixelSizeFromDistanceNM(widthRoughnessParam.designValue, IOPT->getPixelSize());
            // }
        }
        ...
        if (cdsem::checkAmpWidthRoughnessObjectIsDense(widthRoughnessParam.object)) {
            ...
            int distance = cdsem::PixelDistanceConvert::instance()->getPixelSizeFromDistanceNM(widthRoughnessParam.designValue, IOPT->getPixelSize());
            //modify direction
            // if (widthRoughnessParam.direction == 0) {
            measurementTool->setMaxSize({distance + cdsem::AMP_WIDTH_MAX_SEARCH_AREA, cdsem::AMP_WIDTH_MAX_SUM_LINE});
            measurementTool->setMinSize({distance + cdsem::AMP_WIDTH_MIN_SEARCH_AREA, cdsem::AMP_WIDTH_MIN_SUM_LINE});
...
void WidthRoughnessCursorHandler::markWidthLineResultTools() {
    AmpWidthRoughnessParam& widthRoughnessParam = cdsem::CDSEM_GlobalDataHelper::get()->getTmpAmpWidthRoughnessParam();
    // if (widthRoughnessParam.measurePosition != 1) return;
    ...
    if (widthRoughnessParam.cursorType == cdsem::ABCT_TWO_BOX) {
        ...
        int d = cdsem::PixelDistanceConvert::instance()->getPixelSizeFromDistanceNM(widthRoughnessParam.designValue, IOPT->getPixelSize());
        //modify direction
...
void WidthRoughnessCursorHandler::markWidthLineTools() {
    dataHelper->markWidthLineTools();
}

void WidthRoughnessCursorHandler::notifyAmpChanged(void* pPtr) {
    AmpWidthRoughnessParam& widthRoughnessParam = cdsem::CDSEM_GlobalDataHelper::get()->getTmpAmpWidthRoughnessParam();
    if (pPtr == &widthRoughnessParam.measurePosition) {
        dataHelper->stopAutoMeasure();
        return;
    }
    cdsem::AMS_Status amsStatus = cdsem::CDSEM_GlobalDataHelper::get()->getAmsStatus();
    ...
    uiHelper->canvasToolWidget->setVisible(widthRoughnessParam.acdParam.acdType == cdsem::ACDT_MANUAL && widthRoughnessParam.acdParam.target > 1);
    //modify direction
    if (/*pPtr != &widthRoughnessParam.direction && */pPtr != &widthRoughnessParam.designValue &&
        pPtr != &widthRoughnessParam.cursorType && pPtr != &widthRoughnessParam.searchArea &&
        pPtr != &widthRoughnessParam.inspectArea && pPtr != &widthRoughnessParam.object && pPtr != &widthRoughnessParam.acdParam.acdType && pPtr != &widthRoughnessParam.acdParam.target
        && pPtr != &widthRoughnessParam.denseParam.cursorSizeX && pPtr != &widthRoughnessParam.denseParam.cursorSizeY) {
        return;
    }
    if (widthRoughnessParam.cursorType == cdsem::ABCT_TWO_BOX) {
        ...
        for (int i = 0; i < dataHelper->ampCursors.doubleBoxTools.size();  ++ i) {
            ...
            //modify direction
            // if (pPtr == &widthRoughnessParam.direction || widthRoughnessParam.direction != int(d->getOrientation()) - 1) {
            //     if (widthRoughnessParam.direction == 0) {
            ...
            //     }
            // }
            if (pPtr == &widthRoughnessParam.designValue || pPtr == &widthRoughnessParam.cursorType) {
                d->blockSignals(true);
                if (widthRoughnessParam.designValue != nullptr)
                    d->setDesignValueKeepCenter(cdsem::PixelDistanceConvert::instance()->getPixelSizeFromDistanceNM(widthRoughnessParam.designValue, IOPT->getPixelSize()), false);
                d->blockSignals(false);
            }
            if (pPtr == &widthRoughnessParam.searchArea || pPtr == &widthRoughnessParam.cursorType) {
                d->setSearchArea(widthRoughnessParam.searchArea, false);
            }
            if ((pPtr == &widthRoughnessParam.inspectArea || pPtr == &widthRoughnessParam.cursorType)) {
                d->setSumLine(widthRoughnessParam.inspectArea, false);
            }
            if (pPtr == &widthRoughnessParam.cursorType) {
        ...
        dataHelper->markMeasureCursor();
        //modify direction
        if (pPtr == &widthRoughnessParam.denseParam.cursorSizeX || pPtr == &widthRoughnessParam.denseParam.cursorSizeY || pPtr == &widthRoughnessParam.object/* || pPtr == &widthRoughnessParam.direction*/ || pPtr == &widthRoughnessParam.searchArea || pPtr == &widthRoughnessParam.inspectArea || pPtr == &widthRoughnessParam.cursorType || pPtr == &widthRoughnessParam.designValue) {
            ImageMeasurementRectTool* d;
            for (int i = 0; i < dataHelper->ampCursors.oneBoxTools.size(); ++ i) {
                ...
                if (i == 0) {
                    ...
                    if (pPtr == &widthRoughnessParam.object) {
                        if (cdsem::checkAmpWidthRoughnessObjectIsDense(widthRoughnessParam.object)) {
                        ...
                        //modify direction
                    } else if (pPtr == &widthRoughnessParam.cursorType/* || pPtr == &widthRoughnessParam.direction*/ || pPtr == &widthRoughnessParam.designValue) {
                        normalChange = true;
                    }
                    if (normalChange) {
                        int dv = cdsem::PixelDistanceConvert::instance()->getPixelSizeFromDistanceNM(widthRoughnessParam.designValue, IOPT->getPixelSize());
                        //modify direction
                        // if (widthRoughnessParam.direction == 0) {
                ...
                if (cdsem::checkAmpWidthRoughnessObjectIsDense(widthRoughnessParam.object)) {
                    d->setSize({widthRoughnessParam.denseParam.cursorSizeX, widthRoughnessParam.denseParam.cursorSizeY}, false);
                } else {
                    //modify direction
                    // if (widthRoughnessParam.direction == 0) {
                        d->setSize({widthRoughnessParam.searchArea + cdsem::PixelDistanceConvert::instance()->getPixelSizeFromDistanceNM(widthRoughnessParam.designValue, IOPT->getPixelSize()), widthRoughnessParam.inspectArea}, false);
                    // } else {
                        // d->setSize({widthRoughnessParam.inspectArea, widthRoughnessParam.searchArea + cdsem::PixelDistanceConvert::instance()->getPixelSizeFromDistanceNM(widthRoughnessParam.designValue, IOPT->getPixelSize())}, false);
                    // }
                }

                const MeasurementSize& newSize = d->getSize();
                // modify direction
                if (/*pPtr == &widthRoughnessParam.direction && */dataHelper->assistRange.offset.x != nullptr) {
                    // if (widthRoughnessParam.direction == 0) {
...
void WidthRoughnessCursorHandler::sortCursors() {
    AmpWidthRoughnessParam& param = cdsem::CDSEM_GlobalDataHelper::get()->getTmpAmpWidthRoughnessParam();
...
void WidthRoughnessCursorHandler::allAmpCursorToCenter() {
    AmpWidthRoughnessParam& param = cdsem::CDSEM_GlobalDataHelper::get()->getTmpAmpWidthRoughnessParam();
    if (param.acdParam.target <= 1 || param.acdParam.acdType != cdsem::ACDT_MANUAL) {
        return;
    }
...
void WidthRoughnessCursorHandler::lockAcdCursorsMouse(bool lock) {
    AmpWidthRoughnessParam& param = cdsem::CDSEM_GlobalDataHelper::get()->getTmpAmpWidthRoughnessParam();
