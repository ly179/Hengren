...
#include "ui_amp_edge_roughness_edit_controller.h"
...
AmpEdgeRoughnessEditController::AmpEdgeRoughnessEditController(QWidget* parent)
    : QWidget(parent) {
    create();
    createAction();
    initRealTimeEdit();
...
AmpEdgeRoughnessEditController::~AmpEdgeRoughnessEditController() noexcept {
...
void AmpEdgeRoughnessEditController::create() {
    this->param = &cdsem::CDSEM_GlobalDataHelper::get()->getTmpAmpEdgeRoughnessParam();
    ui = new cdsem::UiAmpEdgeRoughnessEditController;
    ui->init(this);
...
void AmpEdgeRoughnessEditController::createAction() {
    ...
    connect(ui->objectCBox->getQComboBox(), &QComboBox::currentIndexChanged, this, [this](int) {
        ...
        bool showDense = cdsem::checkAmpEdgeRoughnessObjectIsDense(val);
    ...
    connect(ui->edgeCBox->getQComboBox(), &QComboBox::currentIndexChanged, this, [this](int){
        ...
        ui->commonSettingController->disableLeft(val != 0);
        ui->commonSettingController->disableRight(val != 1);
    });
    ...
    emit ui->edgeCBox->getQComboBox()->currentIndexChanged(0);
}

void AmpEdgeRoughnessEditController::initRealTimeEdit() {
...
void AmpEdgeRoughnessEditController::refreshAcdType() {
...
void AmpEdgeRoughnessEditController::refreshMeasurePos() {
...
void AmpEdgeRoughnessEditController::refreshData() {
    ...
    ui->dataCBox->loadItemFromDatabase(CBOX_AMP_ROUGHNESS_DATA,
                                       ...
                                       [this, old]() {
                                           int val = ui->objectCBox->currentValue();
                                           if (cdsem::checkAmpEdgeRoughnessObjectIsDense(val)) {
                                               QSet<int> invalids;
                                               invalids.insert(3);
                                               invalids.insert(4);
...
void AmpEdgeRoughnessEditController::refreshDense() {
    bool visible = !cdsem::checkAmpEdgeRoughnessObjectIsDense(ui->objectCBox->currentValue());
    ui->searchAreaLabelContent->setVisible(visible);
    ui->searchAreaTextContent->setVisible(visible);
    ui->measurePointLabelContent->setVisible(visible);
    ui->measurePointInputContent->setVisible(visible);

    ui->inspectAreaLabelContent->setVisible(visible);
    ui->inspectAreaTextContent->setVisible(visible);
    ui->sumLinePointLabelContent->setVisible(visible);
    ui->sumLinePointInputContent->setVisible(visible);
}

void AmpEdgeRoughnessEditController::initAsyncCaller(AsyncCaller* caller) {
    if (this->caller) {
        return;
    }
    this->caller = caller;
...
void AmpEdgeRoughnessEditController::resetToComponent() {
    if (!param) {
        return;
    }
...
QString AmpEdgeRoughnessEditController::collectData(AmpEdgeRoughnessParam& param) {
    ...
    param.designValue = fromTextFieldText(ui->mDesignValueTextField, VEVM_MIN_MODE | VEVM_WITH_PLACE_HOLD);
    param.searchArea
    ...
    if (param.rotCorr < 0) {
        return "Rot. Corr. is not specified.";
    }
